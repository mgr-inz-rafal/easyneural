...
impl Network {
    ...
    pub fn fire(&mut self, neuron_repository: &mut NeuronRepository) {
        for layer_id in 0..self.layers.len() {
            ...
            for neuron_index in 0..self.layers[layer_id].neurons.len() {
                ...
                let new_value = Neuron::fire(neuron_id, &mut neuron_repository.neurons);
                self.set_neuron_value(neuron_id, new_value, neuron_repository);
            }
        }
    }
    ...
    fn set_neuron_value(
        ...
        neuron_repository.neurons[neuron_index].value = Some(value);
    }
}
